 ...
 * 
 *      Modified on 23/10/2013 by Víctor M. Sánchez-Cartagena, Universitat d'Alacant
 */
...
void extract(SentenceAlignment &, int);
void addPhrase(SentenceAlignment &, int, int, int, int, string &, int);
...
int main(int argc, char* argv[])
{
  ...
  while(true) {
    ...
    if (sentence.create( englishString, foreignString, alignmentString, i)) {
      extract(sentence, startingPoint);
      
      startingPoint+=sentence.source.size();
    }
...
void extract(SentenceAlignment &sentence, int startingPoint) {
  ...
  for(int startE=0;startE<countE;startE++) {
	...
	if (!out_of_bounds){
	  ...
	  for(int startF=minF;
		...
		addPhrase(sentence, startE, endE, startF, endF, orientationInfo, startingPoint);
	      }
	    }
	}
      }
...
void addPhrase( SentenceAlignment &sentence, int startE, int endE, int startF, int endF , string &orientationInfo, int startingPoint) {
  // source
  ...
  extractFile << (startingPoint+startF) << " ||| ";
