...
// Default constructor
gzfilebuf::gzfilebuf() : file(NULL), io_mode(std::ios_base::openmode(0)), own_fd(false), buffer(NULL), buffer_size(BIGBUFSIZE), own_buffer(true)
{
...
streambuf::pos_type gzfilebuf::seekoff(off_type off, std::ios_base::seekdir way, std::ios_base::openmode mode)
{
  ...
  if (this->is_open())
    {
      // Clean buffers
      /*		char_type* cur_save = this->gptr();
			char_type* end_save = this->egptr();
			this->setg(buffer, buffer, buffer);*/
      switch(way)
	{
	case ios_base::beg:
	  {
	    ret = pos_type(off_type((long)gzseek(file, (z_off_t)off, SEEK_SET)));
	    this->setg(buffer, buffer, buffer);			// reset buffer
	  }
	  break;
	case ios_base::cur:
	  {
	    char_type* comp_init = this->gptr() + off;
	    if (comp_init < buffer || comp_init > this->egptr())	// underflows / overflows
	      {
		off = comp_init - this->egptr();
		this->setg(buffer, buffer, buffer);		// later produces underflows for read data
	      }else if (off != 0)
		{
		  off = 0;
		  this->setg(buffer, comp_init, this->egptr());
		}
	    ret = pos_type(off_type(gzseek(file, off, SEEK_CUR) + this->gptr() - this->egptr()));
	  }
	  break;
	}
    }
