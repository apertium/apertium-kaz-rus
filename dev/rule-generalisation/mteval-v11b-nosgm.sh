...
#First argument: source text file, with one sentence per line
SRC=$1

#Second argument: reference text file, with one sentence per line
REF=$2

#Third argument: test text file (output of the system being evaluated) with one sentence per line
TEST=$3
...
DIRNAME=`dirname $0`
$DIRNAME/mteval-v11b.pl -r $REF-$$.xml -s $SRC-$$.xml -t $TEST-$$.xml
